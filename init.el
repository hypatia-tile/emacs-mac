...
;; Biome lint diagnostics, as a flymake backend.
;;
;; Why not Biome's LSP: eglot runs one server per buffer (`eglot-alternatives'
;; picks one of several, it does not start them together), and that slot is
;; taken by typescript-language-server, which is the one that knows the types.
;; Flymake has no such limit -- it merges any number of backends -- so Biome
;; comes in through its CLI, and tsserver's type errors and Biome's lint
;; findings end up in one list, under the same C-c ! n / C-c ! p.
;;
;; Why the file on disk and not the buffer text: Biome reports no lint
;; diagnostics at all in stdin mode. With `--stdin-file-path' the lint command
;; turns into a fix operation -- it prints the (unchanged) source on stdout and
;; nothing but "The contents aren't fixed" on stderr, whatever `--reporter'
;; says. Linting a copy in a temp directory does not work either: Biome turns
;; its rule domains on from the nearest package.json, so a file outside the
;; project quietly loses the Next.js and React rules (`noImgElement' stops
;; firing). A copy next to the real file would keep them, but it would also
;; land in the directory `next dev' watches, on every check.
;;
;; So the contract is: Biome reports on the file as last saved. While the
;; buffer is modified the backend reports nothing, which clears its own
;; diagnostics and leaves eglot's alone -- flymake tracks each backend
;; separately -- rather than pointing at lines that have since moved. The
;; check is re-run from `apheleia-post-format-hook' as well as on save, so the
;; findings match the file after apheleia has reformatted and re-saved it.
(defun my-biome-executable ()
  "Return the current project's Biome binary, or nil if there is none.
Biome is a devDependency, so it lives in the project's node_modules and
never on PATH: the flake dev shell provides node and pnpm, not the
project's own binaries."
  (when-let* ((dir (locate-dominating-file (or buffer-file-name default-directory)
                                           "node_modules"))
              (bin (expand-file-name "node_modules/.bin/biome" dir))
              ((file-executable-p bin)))
    bin))

(defun my-biome--position (line column)
  "Return the position in the current buffer of Biome's LINE and COLUMN.
Both are 1-based, and Biome counts columns in characters, not bytes.
The end of a Biome range is exclusive, which is also what flymake wants."
  (save-excursion
    (goto-char (point-min))
    (forward-line (1- line))
...
(defun my-biome--diagnostics (source stdout)
  "Convert Biome's JSON report in STDOUT into diagnostics for SOURCE.
Return nil if the output is not the expected JSON -- Biome writes the
report to stdout and everything else, including the \"experimental
reporter\" notice, to stderr, so anything else means the run failed."
  (let ((report (with-current-buffer stdout
                  (goto-char (point-min))
      ...
      (mapcar
       (lambda (diagnostic)
         (let* ((location (alist-get 'location diagnostic))
                (start (alist-get 'start location))
                (end (alist-get 'end location)))
           (flymake-make-diagnostic
            source
            (my-biome--position (alist-get 'line start) (alist-get 'column start))
            (my-biome--position (alist-get 'line end) (alist-get 'column end))
            ...
            (format "%s (%s)"
                    ...
                    (alist-get 'category diagnostic)))))
       ;; json-parse-buffer returns JSON arrays as vectors.
       (append (alist-get 'diagnostics report) nil)))))

(defvar-local my-biome--process nil
  "The `biome lint' process running for this buffer, if any.")
...
extent of the function that started it."
  (unless (process-live-p process)
    (let ((source (process-get process 'my-biome-source))
          ...
          (stderr (process-get process 'my-biome-stderr))
          (report-fn (process-get process 'my-biome-report-fn)))
      (unwind-protect
          ;; Ignore a run that a newer one has already replaced.
          (when (and (buffer-live-p source)
                     (eq process (buffer-local-value 'my-biome--process source)))
            (funcall report-fn (my-biome--diagnostics source stdout)))
        (kill-buffer stdout)
        (kill-buffer stderr)))))

(defun my-biome-flymake (report-fn &rest _args)
  "Report `biome lint' findings for the current buffer to REPORT-FN.
A flymake backend; see the commentary above for what it does and does
not check."
  (when (process-live-p my-biome--process)
    (kill-process my-biome--process))
  (let ((biome (my-biome-executable))
        (file buffer-file-name)
        (source (current-buffer)))
    (if (or (null biome) (null file) (buffer-modified-p))
        (funcall report-fn nil)
      (let* ((stdout (generate-new-buffer " *biome-lint*"))
             (stderr (generate-new-buffer " *biome-lint-stderr*"))
             (process (make-process
                       :name "biome-lint"
                       :noquery t
                       :connection-type 'pipe
                       :buffer stdout
                       ;; Biome writes the JSON report to stdout and its
                       ;; notices to stderr; keeping them apart is what makes
                       ;; the output parseable.
                       :stderr stderr
                       ;; --max-diagnostics defaults to 20; flymake wants all.
                       :command (list biome "lint" "--reporter=json"
                                      "--max-diagnostics=none" file))))
        (process-put process 'my-biome-source source)
        (process-put process 'my-biome-stdout stdout)
        (process-put process 'my-biome-stderr stderr)
        (process-put process 'my-biome-report-fn report-fn)
        (setq my-biome--process process)
        (set-process-sentinel process #'my-biome--sentinel)))))

(defun my-biome-flymake-setup ()
  "Turn on flymake with the Biome backend, where the project has Biome.
Without it there is nothing to report, and flymake in every unrelated
JavaScript buffer would only add an empty indicator to the mode line.
The check is made once, when the buffer opens, so a project gains the
backend after `pnpm install' the next time a file in it is visited."
  (when (my-biome-executable)
    (add-hook 'flymake-diagnostic-functions #'my-biome-flymake nil t)
    (add-hook 'apheleia-post-format-hook #'flymake-start nil t)
    (flymake-mode 1)))

;; TypeScript and JavaScript only. Biome lints JSON and CSS too, but there its
;; rules are thin next to what the formatter already fixes on save.
(dolist (hook '(typescript-ts-mode-hook tsx-ts-mode-hook js-mode-hook))
  (add-hook hook #'my-biome-flymake-setup))
